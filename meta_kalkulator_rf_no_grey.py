...
REPORT_PATH = "rf_calibrated_triage_report.txt"
MODEL_PATH  = "rf_calibrated_triage_model.joblib"
META_PATH   = "rf_calibrated_triage_meta.json"
RELIABILITY_PNG = "rf_calibration_reliability.png"
...
def find_thresholds(y_true, p_pos, sens_target=SENS_TARGET, spec_target=SPEC_TARGET):
    ...
    # największy thr spełniający sens >= target
    for thr in thresholds:
        tn, fp, fn, tp = confusion_from_threshold(y_true, p_pos, thr)
        sens, _, _, _ = sens_spec_ppv_npv(tn, fp, fn, tp)
        if sens >= sens_target:
            t_out = float(thr)

    # najmniejszy thr spełniający spec >= target
    for thr in thresholds:
        tn, fp, fn, tp = confusion_from_threshold(y_true, p_pos, thr)
        _, spec, _, _ = sens_spec_ppv_npv(tn, fp, fn, tp)
        if spec >= spec_target:
            t_in = float(thr)
            break
...
def triage_report(y_true, p_pos, t_out, t_in):
    """
    Raport triage:
    - coverage OUT/IN/GREY
    - metryki OUT-only (bez GREY)
    - metryki IN-only  (bez GREY)
    - constraint check na progach (sens@t_out, spec@t_in) liczone binarnie
      (tylko do weryfikacji constraintów; nie jest to "triage performance")
    """
    y_true = np.asarray(y_true)
    p_pos = np.asarray(p_pos, dtype=float)
    ...
    # constraint check (binarnie przy progach)
    ...
    return {
        ...
        # tylko weryfikacja constraintów
        "at_t_out": {
...
# --- NOWE: covered-only (OUT ∪ IN), GREY excluded ---
def covered_only_report(y_true, p_pos, t_out, t_in):
    """
    Metryki liczone tylko tam, gdzie model wydał decyzję (OUT lub IN).
    GREY jest wyłączone z ewaluacji.
    OUT => pred=NEG_LABEL
    IN  => pred=POS_LABEL
    """
    y_true = np.asarray(y_true)
    ...
    tri = triage_assign(p_pos, t_out, t_in)
    covered_mask = (tri != "GREY")
    n = len(y_true)
    n_cov = int(np.sum(covered_mask))
    ...
    if n_cov == 0:
        return {
            ...
            "coverage": 0.0,
            "tn": 0, "fp": 0, "fn": 0, "tp": 0,
            "acc": 0.0, "sens": 0.0, "spec": 0.0, "ppv": 0.0, "npv": 0.0
        }

    y_cov = y_true[covered_mask]
    tri_cov = tri[covered_mask]

    y_pred = np.empty(n_cov, dtype=object)
    y_pred[tri_cov == "OUT"] = NEG_LABEL
    y_pred[tri_cov == "IN"]  = POS_LABEL

    tn, fp, fn, tp = confusion_matrix(y_cov, y_pred, labels=[NEG_LABEL, POS_LABEL]).ravel()
    ...
    return {
        "n": int(n),
        "n_covered": int(n_cov),
        "coverage": float(n_cov / n) if n else 0.0,
...
def _pprint_triage(rep, title, fh):
    ...
    print(
        f"Rule-out safety (OUT-only): NPV_out={rep['NPV_out']:.4f}, FN_rate_out={rep['FN_rate_out']:.4f}",
        file=fh
    )
    print(
        f"Rule-in certainty (IN-only): PPV_in={rep['PPV_in']:.4f}, FP_rate_in={rep['FP_rate_in']:.4f}",
        file=fh
    )
    print(
        f"Constraint check (binary): sens@t_out={rep['at_t_out']['sens']:.4f} (>= {SENS_TARGET}), "
        f"spec@t_in={rep['at_t_in']['spec']:.4f} (>= {SPEC_TARGET})",
        file=fh
    )

    # USUNIĘTE: metryki @t_out i @t_in (bo to nie triage, tylko binary cutoff)
    print("\n-- Subset counts (triage) --", file=fh)
...
def _pprint_covered(rep_cov, title, fh):
    print(f"\n=== {title} ===", file=fh)
    print(
        f"Covered n={rep_cov['n_covered']} / {rep_cov['n']} "
        f"({rep_cov['coverage']:.1%})  [OUT+IN only, GREY excluded]",
        file=fh
    )
    print(f"TN={rep_cov['tn']}, FP={rep_cov['fp']}, FN={rep_cov['fn']}, TP={rep_cov['tp']}", file=fh)
    print(
        f"Acc={rep_cov['acc']:.4f}, Sens={rep_cov['sens']:.4f}, Spec={rep_cov['spec']:.4f}, "
        f"PPV={rep_cov['ppv']:.4f}, NPV={rep_cov['npv']:.4f}",
        file=fh
    )
...
# =========================
# OOF calibrated probabilities (threshold learning)
# =========================
def oof_calibrated_probs(X, y, rf_params, seed=RANDOM_STATE):
    ...
    for tr_idx, va_idx in skf.split(X, y):
        ...
        try:
            ...
            cal = CalibratedClassifierCV(base_estimator=rf, method=CAL_METHOD, cv=CAL_CV)
...
# =========================
# DATA LOADING (spójne filtrowanie etykiet)
# =========================
def safe_load_dataset(path):
    df = pd.read_csv(path)

    # ujednolicenie etykiet i filtr do {1-2, 3-4}
...
X_test, y_test, test_df = safe_load_dataset(TEST_PATH) if TEST_PATH else (None, None, None)
...
try:
    ...
    cal_final = CalibratedClassifierCV(base_estimator=rf_final, method=CAL_METHOD, cv=CAL_CV)
...
cov_train_oof = covered_only_report(y_train.values, p_train_oof, t_out, t_in)
cov_train_final = covered_only_report(y_train.values, p_train_final, t_out, t_in)
cov_test = covered_only_report(y_test.values, p_test, t_out, t_in) if p_test is not None else None

# 4) CALIBRATION metryki (na wszystkich obserwacjach; GREY tu nie istnieje)
...
# 5) feature importances – diagnostycznie (średnia po foldach kalibracji)
rf_importances = None
try:
    # sklearn: calibrated_classifiers_ -> list; każdy ma estimator/base_estimator (rf)
    rfs = []
    for cc in cal_final.calibrated_classifiers_:
...
with open(REPORT_PATH, "w", encoding="utf-8") as f:
    print("FINAL TRIAGE + CALIBRATION REPORT (RF calibrated; fixed params)", file=f)
    print("=" * 78, file=f)
    ...
    print("\n" + "-" * 78, file=f)
    print("[TRIAGE RESULTS + COVERED-ONLY]", file=f)

    _pprint_triage(rep_train_oof, "TRAIN (OOF) TRIAGE REPORT", f)
    _pprint_covered(cov_train_oof, "TRAIN (OOF) COVERED-ONLY METRICS", f)

    _pprint_triage(rep_train_final, "TRAIN (FINAL model, in-sample) TRIAGE REPORT (diagnostic)", f)
    _pprint_covered(cov_train_final, "TRAIN (FINAL) COVERED-ONLY METRICS (diagnostic)", f)

    if rep_test is not None:
        _pprint_triage(rep_test, "TEST TRIAGE REPORT", f)
        _pprint_covered(cov_test, "TEST COVERED-ONLY METRICS", f)
    ...
    print("\n" + "-" * 78, file=f)
    print("[FEATURE IMPORTANCE (diagnostic)]", file=f)
...
meta = {
    ...
    # NOWE: covered-only metryki (OUT+IN, bez GREY)
    "covered_only_summary": {
        "train_oof": {k: cov_train_oof[k] for k in ["n", "n_covered", "coverage", "tn", "fp", "fn", "tp", "acc", "sens", "spec", "ppv", "npv"]},
        "train_final": {k: cov_train_final[k] for k in ["n", "n_covered", "coverage", "tn", "fp", "fn", "tp", "acc", "sens", "spec", "ppv", "npv"]},
        "test": ({k: cov_test[k] for k in ["n", "n_covered", "coverage", "tn", "fp", "fn", "tp", "acc", "sens", "spec", "ppv", "npv"]} if cov_test is not None else None),
    }
}
