...
• Ensemble (GB + RF [+ XGB]), META-calibration (Isotonic) on the ensemble output
• Two thresholds:
    - T_low  (rule-out): recall ≥ target_recall_low AND NPV ≥ target_npv_low
                         + minimum rule-out fraction; fallback: Fβ (β=2)
    - T_high (rule-in):  specificity ≥ target_specificity_high + minimum rule-in fraction
• Consistent metrics & plots: CM for T_low and T_high, ROC, histogram with both lines
"""
...
class LiverFibrosisMetaCalculatorF2plusDual:
    ...
    (i.e., no/mild fibrosis vs significant fibrosis ≥F2).
    Mode: DUAL THRESHOLDS (rule-out + rule-in) + Isotonic meta-calibration.
    """
    ...
    def prepare_data(self, df: pd.DataFrame):
        ...
        y = pd.Series(np.where(fs == 1, 0, 1).astype(int), index=df.index, name="y_geF2")
    ...
    def evaluate(self, X, y, plot=True, df_original=None, export_prefix=""):
        y_proba = self._proba(X)

        # --- Extract FALSE NEGATIVES in RULE_OUT (pred <F2, true ≥F2) ---
        # Ensure y is a Series with the same index as X
        if not isinstance(y, pd.Series):
            y = pd.Series(y, index=X.index, name="y_geF2")

        y_proba_s = pd.Series(y_proba, index=X.index, name="p_geF2")

        if self.threshold_low is not None:
            fn_ruleout_mask = (y == 1) & (y_proba_s < self.threshold_low)

            fn_idx = X.index[fn_ruleout_mask]
            print(f"\n[DEBUG] FN in RULE_OUT (<T_low but true ≥F2): {len(fn_idx)} records")

            # Minimal export: features + y + proba
            fn_features = X.loc[fn_idx].copy()
            fn_features["y_true_geF2"] = y.loc[fn_idx]
            fn_features["p_geF2"] = y_proba_s.loc[fn_idx]
            fn_features["meta_score_0_10"] = (y_proba_s.loc[fn_idx] * 10).round(2)

            fn_features.to_csv(f"{export_prefix}FN_ruleout_geF2_features.csv", index=True)

            # Full export: whole original row (if provided)
            if df_original is not None:
                fn_full = df_original.loc[fn_idx].copy()
                fn_full["y_true_geF2"] = y.loc[fn_idx]
                fn_full["p_geF2"] = y_proba_s.loc[fn_idx]
                fn_full["meta_score_0_10"] = (y_proba_s.loc[fn_idx] * 10).round(2)
                fn_full.to_csv(f"{export_prefix}FN_ruleout_geF2_FULLROWS.csv", index=True)
        ...
        if plot:
            ...
            axes[0, 1].set_title('ROC curve (FSCORE 1 vs 2–4)', pad=12)
    ...
    def calculate_score(self, apri, fib4, ritis, nafld):
        """Return score and triage zone for the 1 vs 2–4 (≥F2) task."""
...
def main_pipeline(csv_path, do_plots=True):
    ...
    metrics = calc.evaluate(X_te, y_te, plot=do_plots, df_original=df, export_prefix="test_")
